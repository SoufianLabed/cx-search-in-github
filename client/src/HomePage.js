...
class HomePage extends React.Component{
  ...
  send(){
    if(this.state.nom != ''){
    window.location.href = `http://localhost:3000/#/users/${this.state.nom.toLowerCase()}`
    }else{
      return <div>Entrer une valeur avant</div>
    }
  }
  ...
  render(){
    ...
    return (
      ...
      <div className='main'>
        ...
        <div style={{marginTop:"1%",textAlign:"left",textAlign:"center",fontWeight:"bold"}}><label htmlFor="nom">Nom de user</label></div>
        <div  >   
          
          <input  type="texte" style={{height:'100%'}} id="username" value={this.state.nom} onChange={this.handleChange} />
...
{/*
        ...
        variant="contained"
        color="primary">
          <Link className="link" to={`/users/${this.state.nom}`}>Click to display</Link>
        </Button>*/}
